...
class TabAssignDevice(BaseModel):
    ...
    gb_evt_type_id = Column(INTEGER(11), nullable=False, comment='设备国标类型id')
    gb_evt_type_name = Column(String(32), nullable=False, comment='事件国标类型名称')
    path = Column(String(128), nullable=False, comment='图标地址')
    description = Column(String(128), nullable=False, comment='描述')
    device_status = Column(TINYINT(1), nullable=False, server_default='0', comment='设备状态（0 正常 1 报警）')
...
class TabDevice(BaseModel):
    ...
    setup_date = Column(Date, nullable=False, default=date.today(), comment='装机日期')
    maintain_cycle = Column(INTEGER(11), nullable=False, comment='维保周期')
    expiration_date = Column(Date, nullable=False, default=date.today(), comment='有效期')
    description = Column(String(128), nullable=False, comment='描述')
    path = Column(String(128), comment='设备图标地址')
    is_online = Column(TINYINT(1), nullable=False, server_default='1', comment='是否在线')
    is_fire = Column(TINYINT(1), nullable=False, server_default='0', comment='是否火警（0 否 1 是）')
    is_malfunction = Column(TINYINT(1), nullable=False, server_default='0', comment='是否故障（0 否 1 是）')
    is_vl_malfunction = Column(TINYINT(1), nullable=False, server_default='0', comment='是否声光故障（0 否 1 是）')
    is_feedback = Column(TINYINT(1), nullable=False, server_default='0', comment='是否反馈（0 否 1 是）')
    is_supervise = Column(TINYINT(1), nullable=False, server_default='0', comment='是否监管（0 否 1 是）')
    is_shielding = Column(TINYINT(1), nullable=False, server_default='0', comment='是否屏蔽（0 否 1 是）')
    is_vl_shielding = Column(TINYINT(1), nullable=False, server_default='0', comment='是否声光屏蔽（0 否 1 是）')
    is_linkage = Column(TINYINT(1), nullable=False, server_default='0', comment='是否联动（0 否 1 是）')
    ...
    building = Column(String(32), comment='楼宇名称')
    unit = Column(String(32), comment='单元名称')
    floor = Column(String(32), comment='楼层名称')
    district = Column(String(32), comment='分区名称')
...
class TabFloor(BaseModel):
    ...
    is_alarm = Column(INTEGER(11), nullable=False, comment='是否报警（0 否 1 是）')
...
class TabProject(BaseModel):
    ...
    mobile = Column(String(32), nullable=False, comment='项目联系电话')
...
class TabProjectPicture(BaseModel):
    ...
    id = Column(INTEGER(11), primary_key=True, comment='ID')
    path = Column(String(128), nullable=False, comment='图片地址')
    quick_svg_path = Column(String(128), nullable=False, comment='图片地址（布点后生成的svg图片地址）')
    picture_type_id = Column(INTEGER(11), nullable=False, comment='图片类型id')
    picture_type_name = Column(String(32), nullable=False, comment='图片类型名称')
    project_id = Column(INTEGER(11), nullable=False, comment='项目id')
