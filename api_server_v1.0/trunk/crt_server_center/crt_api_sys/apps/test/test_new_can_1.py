        ...
        while True:
            if self._is_connected:
                try:
                    ...
                    logger.info(
                        f"读到计数 , {number}, {str(threading.current_thread().ident) + str(threading.current_thread().name)}")
                    if number > 0:
                        ...
                        logger.info(
                            f"读取的数据, {n}, {type(data)}, {data}, {str(threading.current_thread().ident) + str(threading.current_thread().name)}")
                        if data:
                            func(data)
                    time.sleep(0.02)
                    logger.info(
                        f"出来了, {number}, {str(threading.current_thread().ident) + str(threading.current_thread().name)}")
                except Exception as e:
                    self._is_connected = False
                    self._serial = None
                    #                    time.sleep(0.01)
