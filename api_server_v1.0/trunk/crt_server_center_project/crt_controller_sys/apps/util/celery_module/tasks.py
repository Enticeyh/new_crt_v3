...
def controller_adjust_time(device_num):
    try:
        # 查询主项目id
        project_id = 0
        project = sync_load_active_project(conn=conn)
        if project:
            project_id = project.id

        controller = sync_load_controller_by_num(project_id=project_id, controller_num=device_num, conn=conn)
...
def controller_report(*_, alarm_time, controller_num, loop_num, addr_num, pass_num, alarm_type_id, gb_evt_type_id,
    ...
    try:
        ...
        if controller_num is not None:
            controller = sync_load_controller_by_num(project_id=project_id, controller_num=controller_num, conn=conn)
            if not controller:
        ...
        try:
            ...
            if controller and loop_num and addr_num:
                device = sync_load_device_by_location(project_id, controller_num, loop_num, addr_num, conn=conn)
            ...
            device_alarm = conn.hget('alarm_list', f'{project_id}-{controller_num}-{loop_num}-{addr_num}-{pass_num or 0}-{alarm_type_id}')
            alarm_log_id_list = conn.hget('alarm_log_id_list', f'{project_id}-{controller_num}-{loop_num}-{addr_num}-{pass_num or 0}-{alarm_type_id}')
            ...
            if device_alarm:
                ...
                if new_gb_evt_type_id in device_alarm:
                    logger.info(f"设备：{project_id}-{controller_num}-{loop_num}-{addr_num}-{pass_num or 0} 已经存在{gb_evt_type.get('name')}，事件抛弃！")
                    return
                # 如果国标事件对应的是消失事件 清除缓存中该设备该报警类型的所有记录
                elif event_state == 0:
                    conn.hdel('alarm_list', f'{project_id}-{controller_num}-{loop_num}-{addr_num}-{pass_num or 0}-{alarm_type_id}')

                    # 清除报警队列对应出现事件
                    for alarm_log_id in alarm_log_id_list:
                        conn.hdel(alarm_log_name, alarm_log_id)
                    del_alarm_log_num = len(alarm_log_id_list)
                    conn.hdel('alarm_log_id_list', f'{project_id}-{controller_num}-{loop_num}-{addr_num}-{pass_num or 0}-{alarm_type_id}')
                # 如果事件为出现且之前没有出现过相同的国标事件 将该国标事件id加入到缓存记录中
                else:
                    device_alarm.append(new_gb_evt_type_id)
                    conn.hset('alarm_list', f'{project_id}-{controller_num}-{loop_num}-{addr_num}-{pass_num or 0}-{alarm_type_id}', json.dumps(device_alarm))
            # 如果没有查到该设备该报警大类的缓存，添加缓存
            else:
                # 如果之前该设备没有该报警大类的出现事件 消失事件抛弃
                if event_state == 0:
                    logger.info(f"设备：{project_id}-{controller_num}-{loop_num}-{addr_num}-{pass_num or 0} 不存在{alarm_type_name_zh}事件，抛弃！")
                    return
                conn.hset('alarm_list', f'{project_id}-{controller_num}-{loop_num}-{addr_num}-{pass_num or 0}-{alarm_type_id}', json.dumps([new_gb_evt_type_id]))
            ...
            with db.slice_session() as session:
                ...
                if event_state == 1:
                    if alarm_log_id_list:
                        ...
                        conn.hset('alarm_log_id_list', f'{project_id}-{controller_num}-{loop_num}-{addr_num}-{pass_num or 0}-{alarm_type_id}', json.dumps(alarm_log_id_list))
                    else:
                        conn.hset('alarm_log_id_list', f'{project_id}-{controller_num}-{loop_num}-{addr_num}-{pass_num or 0}-{alarm_type_id}', json.dumps([alarm_log.id]))
