...
def sync_load_controller_by_num(project_id, controller_num, conn=None, origin=default_origin):
    record = None
...
def sync_load_device_by_location(project_id, controller_num, loop_num, addr_num, conn=None, origin=default_origin):
    record = None
    params_md5 = hash_func_params(project_id=project_id, controller_num=controller_num, loop_num=loop_num, addr_num=addr_num)
    ...
    if not record:
        with db.slice_session() as session:
            qry_func = select(TabDevice).where(TabDevice.is_delete == 0, TabDevice.project_id == project_id,
...
def sync_load_active_project(conn=None, origin=default_origin):
    record = None
