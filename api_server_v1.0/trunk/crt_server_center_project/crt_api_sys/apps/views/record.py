    ...
    async def get(self, request):
        ...
        try:
            records = await async_load_alarm_logs_by_params(redis=self.redis, page=page, per_page=per_page, st=st,
                                                            ...
                                                            project_id=project_id, controller_num=controller_num,
                                                            loop_num=loop_num, addr_num=addr_num,
                                                            equipment_num=equipment_num, module_num=module_num,
                                                            alarm_type_id=alarm_type_id, alarm_status=alarm_status,
                                                            alarm_type=alarm_type, device_id=device_id,
                                                            device_type_ids=device_type_ids, origin='db')
        except Exception as e:
            msg = f'数据库查询错误！'
