...
from . import BaseHandler, CfResponse, get_jsons, ErrorCode, redis_factory, db
...
class TestRedis(BaseHandler):
    async def post(self, request):
        ...
        try:
        #     login_name = get_field("login_name")
        ...
        #     print(people)
            ...
            # keys = ['tab_device-records:b5356c71fd3d7de11c19bc135b773fea']
...
class TestMySql(BaseHandler):
    async def post(self, request):
        try:
            ...
            # 批量修改
            # async with db.slice_session() as session:
            #     delete_role = update(TabRole).where(TabRole.is_delete == 0).values(name="zhangsan")
            #     await session.execute(delete_role)

            # 删除记录
            # async with async_session.begin():
            #     stmt = delete(TabRole).where(TabRole.is_delete == 0)
            #     await async_session.execute(stmt)

            # 批量删除
            # async with db.slice_session() as session:
            #     delete_role = delete(TabRole).where(TabRole.is_delete == 0)
            #     records = await session.execute(delete_role)
        ...
        import json
        from crt_api_sys.apps.util.async_db_api import async_load_device_icon
