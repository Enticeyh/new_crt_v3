...
class ProjectsList(BaseHandler):
    ...
    async def get(self, request):
        ...
        attrs = {'id', 'name', 'address', 'mobile', 'deploy_users', 'is_active'}
    ...
    async def post(self, request):
        ...
        try:
            ...
            address = get_field("address")
            mobile = get_field("mobile")
            is_active = get_field("is_active", default=1, field_type=int)
        ...
        try:
            async with db.slice_session() as session:
                count_qry_func = select(func.count(TabAlarmLog.id)).where(TabProject.is_delete == 0)
                total = (await session.execute(count_qry_func)).scalar()
                if not total and is_active == 0:
                    is_active = 1
        except Exception as e:
            msg = f'数据库操作错误！'
            self.logger.error(msg)
    ...
    async def put(self, request):
        ...
        try:
            ...
            is_active = get_field("is_active", field_type=int)
        ...
        try:
            if is_active is not None:
                async with db.slice_session() as session:
                    qry_func = select(TabProject).where(TabProject.is_delete == 0, TabProject.is_active == 1)
                    projects = await session.execute(qry_func)
                    projects = projects.scalars().all()
                    if len(projects) == 1 and projects[0].id == project_id and is_active == 0:
                        msg = f"系统必须存在主项目，请先指定其他主项目再进行修改！"
                        return await self.write_json(CfResponse(err=ErrorCode.BUSINESS_ERR_PARAMS_INVALID, msg=msg))
        except Exception as e:
            msg = f'数据库操作错误！'
            self.logger.error(msg)
        ...
        try:
            async with db.slice_session() as session:
                ...
                if not project:
                    ...
                    if is_active is not None:
                        project.is_active = int(is_active)
    ...
    async def delete(self, request):
        ...
        try:
            async with db.slice_session() as session:
                ...
                projects = await session.execute(qry_func)
                projects = projects.scalars().all()
                if len(projects) == 1 and projects[0].id == project_id:
                    msg = f"系统必须存在主项目，请先指定其他主项目再进行删除！"
                    return await self.write_json(CfResponse(err=ErrorCode.BUSINESS_ERR_PARAMS_INVALID, msg=msg))
        except Exception as e:
            msg = f'数据库操作错误！'
            self.logger.error(msg)
    ...
    async def get(self, request):
        ...
        try:
            ...
            async with db.slice_session() as session:
                ...
                qry_func = qry_func.order_by(TabController.project_id, TabController.code)
        ...
        try:
            async with db.slice_session() as session:
                ...
                if controllers_code.get(code):
                    msg = f'{project.name} 项目，控制器号 {controllers_code.get(code)} 已存在，请检查提交数据是否正确！'
                    return await self.write_json(CfResponse(err=ErrorCode.DB_ERR_UNCONNECTED, msg=msg))
    ...
    async def put(self, request):
        ...
        try:
            async with db.slice_session() as session:
                ...
                if code:
                    code_qry_func = select(TabController).where(TabController.is_delete == 0)
                    code_qry_func = code_qry_func.where(TabController.project_id == controller.project_id)
                    code_qry_func = code_qry_func.where(TabController.code == code)
                    ...
                    if new_code_controller:
                        msg = f"{new_code_controller.project_name} 项目，控制器号 {code} 已存在，同一项目不能出现相同控制器号！"
                        return await self.write_json(CfResponse(err=ErrorCode.BUSINESS_ERR_PARAMS_INVALID, msg=msg))
            ...
            if table_data:
                async with db.slice_session() as session:
                    ...
                    if repeat_psn:
                        ...
                        msg = f"{project.name} 项目，控制器编号 {repeat_psn} 已存在，请检查数据后重新上传！"
                        self.logger.info(msg)
