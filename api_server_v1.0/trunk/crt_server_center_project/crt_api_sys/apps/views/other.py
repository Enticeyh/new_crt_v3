...
from crt_api_sys.apps.util.async_db_api import async_load_device_by_location, async_load_system_param, \
    ...
    async_load_last_version, async_load_versions, async_load_active_project, \
    async_load_controller_by_num
...
class MockTest(BaseHandler):
    ...
    async def post(self, request):
        ...
        try:
            project_id = get_field("project_id", field_type=int)  # 项目id
        ...
        try:
            ...
            if not project_id:
                project = await async_load_active_project(redis=self.redis)
                if not project:
                    msg = '主项目不存在！'
            ...
            controller = await async_load_controller_by_num(project_id=project_id, controller_num=controller_num, redis=self.redis)
            if not controller:
                msg = '{controller_num} 控制器不存在！'
                self.logger.info(msg)
                return await self.write_json(CfResponse(err=ErrorCode.BUSINESS_ERR_PARAMS_INVALID, msg=msg))
