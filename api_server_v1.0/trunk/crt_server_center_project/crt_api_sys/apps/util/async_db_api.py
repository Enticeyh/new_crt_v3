...
async def async_load_controller_by_num(project_id, controller_num, redis=None, origin=default_origin):
    record = None
    params_md5 = hash_func_params(project_id=project_id, controller_num=controller_num)
    k = f'tab_controller-record:{params_md5}'
    if origin == 'redis':
        record = await redis_factory.get_string_cache(k, to_json=True, redis=redis)
        logger.debug(f'load from redis: {record}')
    ...
    if not record:
        async with db.slice_session() as session:
            ...
            qry_func = qry_func.where(TabController.project_id == project_id, TabController.code == controller_num)
            record = await session.execute(qry_func)
            record = record.scalars().first()
            record = record.to_dict() if record else None
...
async def async_load_alarm_logs_by_params(redis=None, origin='db', **kwargs):
    ...
    if not result:
        ...
        async with db.slice_session() as session:
            ...
            if project_id:
                count_qry_func = count_qry_func.filter(TabAlarmLog.project_id == project_id)
                qry_func = qry_func.filter(TabAlarmLog.project_id == project_id)
...
async def async_load_controller_op_logs_by_params(redis=None, origin=default_origin, **kwargs):
    ...
    if not result:
        ...
        async with db.slice_session() as session:
            ...
            qry_func = qry_func.order_by(TabControllerOpLog.create_time.desc())
...
async def async_load_maintenance_logs_by_params(redis=None, origin=default_origin, **kwargs):
    ...
    if not result:
        ...
        async with db.slice_session() as session:
            ...
            qry_func = qry_func.order_by(TabMaintenanceLog.create_time.desc())
...
async def async_load_shift_records_by_params(redis=None, origin=default_origin, **kwargs):
    ...
    if not result:
        ...
        async with db.slice_session() as session:
            ...
            qry_func = qry_func.order_by(TabShiftRecord.create_time.desc())
...
async def async_load_system_logs_by_params(redis=None, origin=default_origin, **kwargs):
    ...
    if not result:
        ...
        async with db.slice_session() as session:
            ...
            qry_func = qry_func.order_by(TabSystemLog.create_time.desc())
...
async def async_load_last_version(redis=None, origin=default_origin):
    ...
    if not record:
        async with db.slice_session() as session:
            ...
            qry_func = qry_func.order_by(TabVersion.id.desc())
            record = await session.execute(qry_func)
            record = record.scalars().first()
            record = record.to_dict() if record else None
...
async def async_load_active_project(redis=None, origin=default_origin):
    """
    查询主项目
    :param redis:
    :param origin:
    ...
    """
    record = None
    params_md5 = hash_func_params(is_active=1)
    k = f'tab_project-record:{params_md5}'
    if origin == 'redis':
        record = await redis_factory.get_string_cache(k, to_json=True, redis=redis)
        logger.debug(f'load from redis: {record}')
