...
from sqlalchemy import select, Column, DateTime, TIMESTAMP, func
...
class BaseModel(Base):
    ...
    @classmethod
    async def async_fetch_record_size(cls, session):
        """
            查询记录总数
    ...
    @classmethod
    async def async_fetch_latest_record(cls, session):
        """
            查询最新一条记录
        :param session: db session
        :return:
    ...
    @classmethod
    async def async_fetch_record_by_id(cls, record_id, session):
        """
    ...
    @classmethod
    async def async_fetch_record_by_ids(cls, record_ids, session):
        """
            根据 record_id 查询记录, 可携带分片键提高效率
        :param record_ids:
        :param session: db session
    ...
    @classmethod
    async def async_fetch_records(cls, session, limit=None):
        """
            查询所有的记录
        :param session: db session
        :param limit:
