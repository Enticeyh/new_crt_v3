    ...
    if not is_init:
        ...
        async with db.slice_session() as session:
            ...
            for alarm_log in alarm_logs:
                ...
                device_alarm_key = f'{alarm_log.controller_num}{alarm_log.loop_num or 0}-{alarm_log.addr_num or 0}-{alarm_log.loop_num}-{alarm_log.addr_num}-{alarm_log.pass_num or 0}-{alarm_log.alarm_type_id}'
                ...
                if alarm_log.alarm_status == 1:
                    ...
                    if alarm_log.alarm_type_id == 1:
                        ...
                        if alarm_info_statistics['first_fire'] == 0:  # 首警
                            ...
                            await redis.hset('alarm_info_statistics', 'contingency_plan', 1)  # 预案启动
            ...
            qry_func = select(TabControllerOpLog).where(TabControllerOpLog.is_delete == 0, TabControllerOpLog.snow_id > last_reset_snow_id)
            qry_func = qry_func.where(TabControllerOpLog.gb_evt_type_id.in_([306, 307, 308, 309]))
            qry_func = qry_func.order_by(TabControllerOpLog.snow_id)
            controller_op_logs = await session.execute(qry_func)
            controller_op_logs = controller_op_logs.scalars().all()

            for controller_op_log in controller_op_logs:
                if controller_op_log.gb_evt_type_id == 306:
                    alarm_info_statistics['forced_light_on'] = 1
                elif controller_op_log.gb_evt_type_id == 307:
                    alarm_info_statistics['forced_light_on'] = 0
                elif controller_op_log.gb_evt_type_id == 308:
                    alarm_info_statistics['forced_emergency'] = 1
                elif controller_op_log.gb_evt_type_id == 309:
                    alarm_info_statistics['forced_emergency'] = 0
