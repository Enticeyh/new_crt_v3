...
class TestCenter(BaseHandler):
    ...
    async def get(self, request):
        ...
        url = request.app.config.CENTER_TEST_URL % (center.ip, center.port)
        data = {
            'code': center.code,
    ...
    async def get(self, request):
        ...
        try:
            async with db.slice_session() as session:
                ...
                # 在指定路径存在，删除路径
                if Path(f'{path}/smart_iot_data').exists():
                    shutil.rmtree(f'{path}/smart_iot_data')

                # 生成资源路径
                self.logger.info("生成资源路径！")
                os.makedirs(f'{path}/smart_iot_data')
                os.makedirs(f'{path}/smart_iot_data/static')

                # 将楼层图复制到指定目录
                try:
                    # 复制静态文件
                    old_file = f'{request.app.config.STATIC_ROOT_PATH}/static/photo'
                    new_file = f'{path}/smart_iot_data/static/photo'
                ...
                try:
                    ...
                    excel_path = f'{path}/smart_iot_data/crt_data.xls'
                    # 创建Excel文件,不保存, 直接输出
        ...
        return await self.write_json(CfResponse(data=f'导出数据路径为：{path}/smart_iot_data'))
    ...
    async def get(self, request):
        ...
        try:
            ...
            if len(backups_dir_list) >= 5:
                backups_dir_list = sorted(backups_dir_list)
...
class Upgrade(BaseHandler):
    ...
    async def post(self, request):
        ...
        try:
            path = get_field("path")
            backups_path = get_field("backups_path")
        ...
        try:
            ...
            os.system(f"cd {root_path} && cd .. &&  zip -q -r {zip_name} {root_path_name} && mv {zip_name} {backups_zip_path}  && rm {root_path}/*backup.sql && rm {is_upgraded_backup}")
        ...
        try:
            ...
            if 'version.txt' in dir_list:
                ...
                with open(version_path, 'r') as f:
                    ...
                    version_info = json.dumps([line.replace('\n', '') for line in data[2:]])
                ...
                async with db.slice_session() as session:
                    ...
                    if all_table_data.get('table_project'):
                        ...
                        for raw_data in table_data:
                            ...
                            session.add(TabProject(**area))
                            await session.flush()
                            project_id = area.get('id')
                            project_name = area.get('name')
        ...
        # 睡眠40秒 解决CRT升级成功立刻断电重启导致的丢失文件问题
        time.sleep(40)

        return await self.write_json(CfResponse(data=f"备份路径为：{backups_zip_path}"))
...
class LastVersion(BaseHandler):
    ...
    async def get(self, _):
        ...
        if record:
            ...
            record['notes'] = json.loads(record.get('notes'))
    ...
    async def get(self, request):
        ...
        if records['items']:
            for record in records['items']:
                record['upgrade_time'] = record.get('create_time')
                record['notes'] = json.loads(record.notes)
