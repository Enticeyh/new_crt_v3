...
async def async_load_controller(page=1, per_page=10, redis=None, origin=default_origin):
    records = []
    k = f'tab_controller-records:by_num'
    if origin == 'redis':
        records = await redis_factory.get_hash_cache(k, redis=redis)
        logger.debug(f'load from redis: {records}')
    ...
    if not records:
        ...
        if origin == 'redis' and records:
            records = {record.code: json.dumps(record.to_dict(is_attr_dict=False)) for record in records}
            await redis.hset(k, mapping=records)
