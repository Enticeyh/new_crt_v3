    ...
    async def post(self, request):
        ...
        try:
            async with db.slice_session() as session:
                ...
                _project = TabProject(**project)
                session.add(_project)
                await session.flush()

                # 创建同名小区
                area = {
                    "name": f'{_project.name}小区',
    ...
    async def delete(self, request):
        ...
        try:
            async with db.slice_session() as session:
                ...
                try:
                    self.logger.info(f'删除项目图片路径：{request.app.config.STATIC_ROOT_PATH}{project_picture.path}')
                    os.remove(f'{request.app.config.STATIC_ROOT_PATH}{project_picture.path}')
                except Exception as _:
                    self.logger.error(f'删除项目图片失败！{request.app.config.STATIC_ROOT_PATH}{project_picture.path}')
        ...
        try:
            async with db.slice_session() as session:
                ...
                if not build:
                    ...
                    if picture:
                        # 检查老的图片是否有其他楼宇在用，没有的话删除图片文件
                        qry_func = select(TabBuild).where(TabBuild.is_delete == 0, TabBuild.path == build.path)
                        build_path = await session.execute(qry_func)
                        build_path = build_path.scalars().first()
                        if not build_path:
                            try:
                                # 图片没有其他楼宇在使用，删除楼宇图片文件
                                self.logger.info(f'删除楼宇图片路径：{request.app.config.STATIC_ROOT_PATH}{build.path}')
                                os.remove(f'{request.app.config.STATIC_ROOT_PATH}{build.path}')
                            except Exception as _:
                                self.logger.error(f'删除楼宇图片失败！{request.app.config.STATIC_ROOT_PATH}{build.path}')
        ...
        try:
            async with db.slice_session() as session:
                ...
                if floors:
                    ...
                    # 一次删除整栋的楼层，所以不存在楼层图被其他地方使用的可能，直接去重删除 快速svg图也是
                    del_image_path = [floor.path for floor in floors if floor.path] + [floor.quick_svg_path for floor in floors if floor.quick_svg_path]
                    del_image_path = list(set(del_image_path))
                    for path in del_image_path:
                        try:
                            self.logger.info(f'删除楼层图片路径：{request.app.config.STATIC_ROOT_PATH}{path}')
                            os.remove(f'{request.app.config.STATIC_ROOT_PATH}{path}')
                        except Exception as _:
                            self.logger.error(f'删除楼层图片失败！{request.app.config.STATIC_ROOT_PATH}{path}')
        ...
        try:
            async with db.slice_session() as session:
                ...
                if not floor:
                    ...
                    if picture:
                        # 检查老的图片是否有其他楼层在用，没有的话删除图片文件
                        qry_func = select(TabFloor).where(TabFloor.is_delete == 0, TabFloor.path == floor.path)
                        floor_path = await session.execute(qry_func)
                        floor_path = floor_path.scalars().first()
                        if not floor_path:
                            try:
                                # 图片没有其他楼宇在使用，删除楼宇图片文件
                                self.logger.info(f'删除楼层图片路径：{request.app.config.STATIC_ROOT_PATH}{floor.path}')
                                os.remove(f'{request.app.config.STATIC_ROOT_PATH}{floor.path}')
                            except Exception as _:
                                self.logger.error(f'删除楼层图片失败！{request.app.config.STATIC_ROOT_PATH}{floor.path}')
                    ...
                    if quick_svg:
                        if floor.quick_svg_path:
                            try:
                                self.logger.info(f'删除快速楼层图片路径：{request.app.config.STATIC_ROOT_PATH}{floor.quick_svg_path}')
                                os.remove(f'{request.app.config.STATIC_ROOT_PATH}{floor.quick_svg_path}')
                            except Exception as _:
                                self.logger.error(f'删除快速楼层图片失败！{request.app.config.STATIC_ROOT_PATH}{floor.quick_svg_path}')
        ...
        try:
            async with db.slice_session() as session:
                ...
                for floor in floors:
                    ...
                    del_image_path.append(floor.path)
                ...
                del_image_path = list(set(del_image_path))
                for path in del_image_path:
                    qry_func = select(TabFloor).where(TabFloor.is_delete == 0, TabFloor.path == path)
                    floor_path = await session.execute(qry_func)
                    floor_path = floor_path.scalars().first()
                    if not floor_path:
                        try:
...
class Controllers(BaseHandler):
    ...
    async def get(self, request):
        ...
        # 通过缓存取数据
        # result = await async_load_controller(page=page, per_page=per_page, redis=self.redis)
        # result['items'] = sorted(result['items'], key=lambda x: x.get('code'))

        attrs = {'id', 'name', 'code', 'model', 'manufacturer', 'setup_date', 'controller_type', 'host_id',
                 'is_online', 'power_type', 'project_id', 'project_name', 'is_shielding'}
        result['items'] = [{k: record[k] for k in attrs if k in record} for record in result.get('items')]
    ...
    async def put(self, request):
        ...
        try:
            ...
            is_shielding = get_field("is_shielding", field_type=int)  # 是否屏蔽控制器 0 否 1 是 如果屏蔽，控制器的所有事件都不再接收（复位、报警等）

            if not controller_id:
                return await self.write_json(CfResponse(err=ErrorCode.BUSINESS_ERR_PARAMS_LACK, status=400))

            if (is_shielding is not None) and (is_shielding not in [0, 1]):
                return await self.write_json(CfResponse(err=ErrorCode.BUSINESS_ERR_PARAMS_LACK, status=400))
        ...
        try:
            async with db.slice_session() as session:
                ...
                if (is_shielding is not None) and (is_shielding != controller.is_shielding):
                    controller.is_shielding = is_shielding
                ...
                async with db.slice_session() as session:
                    ...
                    for raw_data in table_data:
                        ...
                        if raw_data.get('psn') and len(raw_data.get('psn')) >= 4 and device_icons.get(
                            ...
                            device_type_id = int(CrtConstant.DEVICE_TYPE_DICT.get(raw_data.get('type'), ['0', ''])[0])
