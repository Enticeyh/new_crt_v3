    ...
    <div class="systemRecordBox">
        ...
        <div class="recordBox" v-if="recordData.propsRecordType == 19">
            <div class="timeBox">
                ...
                <div class="timeMsg">间隔时间（分钟）<el-input v-model="recordData.timeInput" clearable placeholder="请输入内容">
                    </el-input>
                </div>
            </div>
            <div class="alarmsListBox">
                ...
                <div class="timeMsg">距上次提示的报警数（条）<el-input v-model="recordData.numInput" clearable placeholder="请输入内容">
                    </el-input>
                </div>
            </div>
            <div class="printBox">
                <div class="printBtn" @click="printClick()">
                    保存
                </div>
            </div>
        ...
        <div class="recordBox" v-if="recordData.propsRecordType == 20">
            <div class="parameterBox">
                <div class="inputBox">&nbsp;&nbsp;&nbsp;故障时长（秒）<el-input v-model="recordData.malfunctionTime"
                        placeholder="请输入内容" clearable>
                    </el-input>
                </div>
                <div class="inputBox">&nbsp;&nbsp;
                    轮播时长（秒）<el-input v-model="recordData.carouselTime" placeholder="请输入内容" clearable>
                    </el-input>

                </div>
                <div class="loTime">轮播时长最低为10秒</div>
                <div class="inputBox">
                    屏保时间（分钟）<el-input v-model="recordData.screenTime" placeholder="请输入内容" clearable>
                    </el-input>
                </div>
                <div class="btnBox">
        ...
        <div class="recordBox" v-if="recordData.propsRecordType == 21" v-loading="recordData.loading">
            <div class="deriveBigBox">
                ...
                <div class="dataBox">
                    ...
                    <div class="deriveBox">
                        导出路径 &nbsp;<el-input @dblclick="fileCopy" v-model="recordData.objectInput" placeholder="请输入导出路径"
                            id="exportBlur" @change="exportBlurFun">
                        </el-input>
                    </div>
                    <el-checkbox v-model="recordData.inherit" label="是否系统备份" @change="inherit" border>
                    </el-checkbox>
            ...
            <div class="upBox">
                <div class="updataBigBox">
                    <div class="upgradeBox">
                        ...
                        <div class="updataBox">
                            <template v-if="recordData.fileDataSelect == 2">
                                升级文件&nbsp;<el-input @dblclick="onCopy" v-model="recordData.updataInput"
                                    placeholder="请输入升级文件指定路径" id="updata" @change="updataBlurFun">
                                </el-input>
                            </template>
                            <template v-else>
                                导入文件&nbsp;<el-input @dblclick="onCopy" v-model="recordData.updataInput"
                                    placeholder="请输入导入文件指定路径" id="updata" @change="updataBlurFun">
                                </el-input>
                            </template>
                        ...
                        <div class="updataMsgBox" :title="recordData.VersionData.notes">版本号:
                            <span style="color: #6a737b;"> {{ recordData.VersionData.version_num }}
                            </span>
                        </div>
                    </div>
                </div>
        ...
        <el-dialog v-model="recordData.updataShow" :show-close="false">
            ...
            <div class="logoutMsg">
                <div class="password">
                    <template v-if="recordData.upState == true">
                        <span style="color: red;">升级完成,请立即重启设备</span>
                    </template>
                    <template v-else>
                        <span style="color: red;">升级失败,请联系管理员确认</span>
                    </template>
                </div>
            </div>
            <div class="sureBox">
...
export default {
    ...
    setup(props) {
        ...
        let recordData = reactive({
            ...
            projectSelect: "",//项目选择
            projectList: [],//
            system_backups: 0,//数据导出-是否选择系统备份
            objectId: "",//项目id
            objectInput: "",//数据导出-导出路径
            updataInput: "",
            backupsInput: "",
            backupInput: "",//备份地址
            // 远程传输
            ...
            carouselTime: "",
            updataShow: false,
            resetShow: false,//升级 重置弹窗
            fireTime: 0,
            upShow: false,
            VersionData: {},//版本号最新
            upState: true,
            timeInput: 20,//打印设置-间隔时间
            numInput: 1000,//打印设置-报警条数
            malfunctionTime: 20,//参数设置-故障时长
            screenTime: 30,//参数设置-屏保时间

        })
        onMounted(() => {
            firstRequest()
            console.log('当前版本为3.0.6');
        })
        const onCopy = () => {
        ...
        const inherit = () => {
            if (recordData.inherit == true) {
                recordData.system_backups = 1
            } else {
                recordData.system_backups = 0
            }
        }
        // 打开弹窗
        ...
        const selectFun = (val) => {
            ...
            if (recordData.objectInput != "") {
                ...
                if (val) {
                    recordData.objectInput = "/home/nanjingcrt/Desktop/导出数据"
                } else {
                    recordData.objectInput = ""
                }
            }
        ...
        const derive = () => {
            ...
            let data = {
                ...
                is_system_backups: recordData.system_backups
            }
            if (recordData.objectInput != "" && recordData.objectInput != "/home/nanjingcrt/Desktop/导出数据") {
                data.path = recordData.objectInput
            }
        ...
        const up = () => {
            ...
            if (recordData.updataInput == "") return ElMessage({
                message: '请填写地址',
                type: 'error',
                duration: 3 * 1000
            })
            ...
            if (recordData.fileDataSelect == 2) {
                ElMessage({
            ...
            let data = {
                ...
                upgrade_type: Number(recordData.fileDataSelect)
            }
            if (recordData.fileDataSelect == "") {
                data.upgrade_type = 1
            }
            let timer = null
            recordData.fireTime = 0
            timer = setInterval(() => {
                if (recordData.fireTime < 90) {
                    recordData.fireTime += 1
                } else if (recordData.fireTime == 90) {
                    recordData.fireTime = 90
                }
            }, 3000);
                ...
                if (res.ok) {
                    ...
                    timerTwo = setInterval(() => {
                        ...
                        if (recordData.fireTime >= 100) {
                            ...
                            recordData.upShow = false
                            recordData.updataShow = true
                            recordData.upState = true
                            recordData.updataInput = ""
                            // recordData.backupsInput = ""
                            recordData.fireTime = 0
                            clearTimeout(timerTwo)
                        } else if (recordData.fireTime == 90) {
                            recordData.fireTime = recordData.fireTime + 10
                        } else {
                            recordData.fireTime += 90 - recordData.fireTime
                        }
                    }, 2000);
                    // 每次进入升级页面时 查询当前版本
        ...
        // }
        // 监听弹窗状态
        watch(() => recordData.updataShow, (oldvalue, newvalue) => {
            if (newvalue == true) {
                recordData.upShow = false
                recordData.loading = false
                recordData.upState = true
        ...
        watch(props, (newProps) => {
            ...
            } else if (newProps.systemRecordState == 21) {
                recordData.system_backups = 0
