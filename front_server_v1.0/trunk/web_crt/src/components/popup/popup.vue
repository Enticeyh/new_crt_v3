    ...
    <div class="popupBox">
        ...
        <el-dialog v-model="popupData.loginShow" :show-close="false">
            ...
            <div class="loginBox">
                ...
                <template v-else-if="popupType == 3">
                    ...
                    <div class="changeBox">
                        <span style="color: red;">*</span>换班密码 <el-input v-model="popupData.shiftPassword" id="newpassword"
                            @change="newpasswordBlur" placeholder="请输入换班密码" type="password">
                        </el-input>
                    </div>
                </template>
        ...
        <el-dialog v-model="popupData.newObjectShow" :show-close="false">
            ...
            <template v-if="popupType == 7">
                <div class="loginBox">
                    <div class="account">
                        <span style="color: red;">*</span>项目名称 <el-input v-model="popupData.addObjectData.name" clearable
                            placeholder="请输入项目名称">
                        </el-input>
                    </div>
                    <div class="password">
                        <span style="color: red;">*</span>项目地址 <el-input v-model="popupData.addObjectData.address" clearable
                            placeholder="请输入项目地址">
                        </el-input>
                    </div>
                    <div class="password">
                        &nbsp;项目电话 <el-input v-model="popupData.addObjectData.mobile" clearable placeholder="请输入项目电话">
                        </el-input>
                    </div>
                    <!-- <div class="password">
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;主项目 <el-select v-model="popupData.addObjectData.is_active" placeholder="请选择项目主副" clearable>
        ...
        <el-dialog v-model="popupData.redactObjectShow" :show-close="false">
                ...
                <div class="loginBox">
                    ...
                    <div class="password">
                        项目人员 <el-select v-model="popupData.userSelect" multiple collapse-tags collapse-tags-tooltip
                            ...
                            clearable>
                            <el-option v-for="item in popupData.userList" :key="item.role_id" :label="item.user_name"
                                :value="item.id" />
                        </el-select>
                    </div>
                </div>
            ...
            <template v-else-if="popupType == 16">
                <div class="loginBox">
                    ...
                    <div class="password">
                        <input type="file" id="newFloor" @change="onchangeFloor" accept=".svg" placeholder="上传文件">
                    </div>
                </div>
            </template>
            <!-- 修改用户 -->
            ...
            <el-dialog v-model="popupData.fastEstablishShow" :show-close="false">
                ...
                <template v-else-if="popupType == 11">
                    <div class="fastEstablishMsg">
                        <div class="fastBox">
                            <span style="color: red;">*</span>小区名称 &nbsp;
                            <el-select v-model="popupData.areaSlect" disabled :placeholder="popupData.popupFData.area_name"
                                clearable @change="selectFun(popupData.areaSlect, 1)">
                                <el-option v-for="item in popupData.areaList" :key="item.id" :label="item.name"
                                    :value="item.id" />
                            </el-select>
                            &nbsp; <span style="color: red;">*</span>楼宇名称 &nbsp;
                            <el-select v-model="popupData.buildingSlect" disabled :placeholder="popupData.popupFData.name"
                                clearable @change="selectFun(popupData.buildingSlect, 2)">
                            </el-select>
                        </div>
                        <div class="fastBox">
                        ...
                        <div class="fastBox">
                            ...
                            <input id="newFloor" type="file" @change="onchangeFloor" accept=".svg" placeholder="上传文件">
                        </div>
                    </div>
                    <div class="sureBox">
                        <button class="sureBtn" @click="sure">确定</button>
                ...
                <template v-else-if="popupType == 14">
                    <div class="fastEstablishMsg">
                        <div class="fastBox">项目名称 &nbsp;<el-select v-model="popupData.projectSelect" placeholder="请选择项目"
                                clearable @change="selectFun(popupData.projectSelect, 1)" @visible-change="visibleObject">
                                <el-option v-for="item in popupData.projectList" :key="item.id" :label="item.name"
                                    :value="item.id" />
                            </el-select>
                        </div>
                    ...
                    <div class="controllerMsg">
                        ...
                        <div class="controllerBox">
                            ...
                            <el-select v-model="popupData.controllerTypeSelect" clearable>
                                <el-option v-for="item in popupData.controllerTypeArr" :key="item.value" :label="item.label"
                                    :value="item.value" />
                            </el-select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        ...
                        <div class="controllerBox">
                            <el-checkbox v-model="popupData.switchValue"  :disabled="popupData.switchValueDisabled" title="超级管理员可修改">  控制器屏蔽</el-checkbox>
                        </div>
                    </div>
                </template>
                <!-- 设备编辑弹窗 -->
                <template v-else-if="popupType == 19">
                    <div class="deviceMsg">
                        <div class="deviceBox">
                            ...
                            地址号 &nbsp;
                            <input class="deviceInput" type="text" :disabled="true"
                                :placeholder="popupData.popupFData.addr_num" style="backgroundColor: #D5D8E4;">&nbsp;&nbsp;
                            设备类型 &nbsp;
                        ...
                        <div class="deviceBox">
                            制造商 &nbsp;
                            <input class="deviceInput" type="text" :disabled="true"
                                :placeholder="popupData.popupFData.manufacturer" style="backgroundColor: #D5D8E4;">&nbsp;
                            安装日期 &nbsp;
                            <input class="deviceInput" type="text" :disabled="true"
                        ...
                        <div class="deviceRemark">
                            设备图标&nbsp;
                            <div class="deviceImg">
                                <img class="deviceimg" :src="popupData.url + popupData.popupFData.path" alt=""
                                    @dragstart.prevent>
                            </div>
                            备注 &nbsp;
                            <div class="remarkInput">
...
export default {
    ...
    setup(props, context) {
        let popupData = reactive({
            ...
            switchValue: false,//控制器屏蔽操作
            switchValueDisabled: true
        })
        ...
        const onchangeFloor = (event) => {
            if (event.target.files[0].type != "image/svg+xml") {
                ...
                event = {}
                popupData.fileList = []
                let file = document.getElementById('newFloor');
                file.value = '';

            } else if ((event.target.files[0].size / 1024 / 1024) > 25) {
                ElMessage({
                    message: '图纸文件过大,请上传小于25M的图纸文件。',
                    type: 'error',
                    duration: 3 * 1000
                })
                event = {}
        ...
        const sure = (val) => {
            if (props.popupType == 1) {
                ...
                let file = document.getElementById('newFloor');
                if (file.value == "") { popupData.fileList = [] }
                let data = {
        ...
        watch(props, (newProps) => {
            ...
            if (newProps.popupType == 1 || newProps.popupType == 3) {
                ...
                if (newProps.popupType == 18) {
                    ...
                    if (props.popupData.is_shielding == 0) {
                        popupData.switchValue = false
                    } else {
