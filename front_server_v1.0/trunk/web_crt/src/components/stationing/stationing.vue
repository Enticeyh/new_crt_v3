    ...
    <div class="stationBox" id="stationBox" v-show="stationState" v-loading="recordData.loading">
                    ...
                    <div class="inheritFloorlistBox" v-loading="recordData.inheritFloorlistLoading">
                        <div class="inheritFloorBox" v-for="item in recordData.inheritList"
                            ...
                            <div class="inheritFloorImg">
                                <img class="inheritImg" :src="recordData.Url + item.path" alt="">
                            </div>
                ...
                <div class="typeBox">
                    <div class="typeName" :class="recordData.switchDeviceTypeActive == 1 ? 'active' : 'unactive'"
                        @click="switchDeviceType(1)" title="控制器-回路">控制器 <img @click="switchDevice(1)"
                            class="switchDeviceImg" src="../../assets/img/comment/refresh.svg" alt=""></div>
                        ...
                        <template v-else-if="recordData.switchDeviceTypeActive == 2">
                            <div class="deviceList">
                                <el-collapse accordion @change="getout">
                                    <el-collapse-item v-for="item in recordData.areaData" @click="getDeviceList(item)">
                                        <template #title>
        ...
        <div class="station" ref="assingPicRef">
            <div class="demandBox" id="demandBox">
                ...
                <!-- 此功能暂不开放使用 等待技术部商讨结束再做改进 -->
                <template v-if="recordData.roleId != 3">
                    <div class="btnBox" @click="showInherit">继承</div>
                </template>
                <template v-if="recordData.createUpdateDevcieIds.length == 0">
                    <div class="btnBox" style="background-color: gray;">保存({{ recordData.createUpdateDevcieIds.length
                    }})</div>
                </template>
                <template v-else>
                    <div class="btnBox" style="background-color: #4a5cd5;" @click="openSavePopup">保存({{
                            recordData.createUpdateDevcieIds.length
                    }})</div>
                </template>
        ...
        <el-dialog v-model="recordData.savePopupShow" :show-close="false">
            ...
            <div class="loginBox">
                ...
                <template v-if="recordData.roleId != 3">
                    <div class="account">
                        <el-checkbox v-model="recordData.inherit" label="是否作为继承模板" @change="inherit"
                            border></el-checkbox>
                    </div>
                </template>
        ...
        <el-dialog v-model="recordData.sureSavePopupShow" :show-close="false">
            <template #header>
                <div class="headerBox">布点保存</div>
                <div class="offBtn" @click="offSureSavePopup">✖</div>
            </template>
            <div class="loginBox">
                <div class="account">
                    当前图纸有暂未保存点位,是否保存?
            ...
            <div class="sureBox">
                <button class="sureBtn" @click="offSurePopup">不保存</button>
                <button class="sureBtn" @click="openSavePopup">保存</button>
            </div>
        </el-dialog>
...
export default ({
    ...
    setup(props) {
        ...
        let recordData = reactive({
            ...
            sureSavePopupShow: false,//确认保存弹窗状态
            selectVal: 0,
            selectType: 0,
            sureSaveState: true,//判断是否保存
        })
        onMounted(() => {
            recordData.roleId = sessionStorage.getItem("role_id")
        })
        // 切换布点设备类型分类
        const switchDeviceType = (type) => {
            ...
            recordData.deviceList = []
        }
        const switchDevice = (type) => {
            recordData.deviceList = []
            let data = {
                page: 0,
                project_id: recordData.projectSlect
            }
            if (type == 1) {
                // 获取控制器层级关系
                controllerLoopRequest(data).then((res) => {
        ...
        const selectFun = (val, type) => {
            if (type == 1) {
                ...
                recordData.selectVal = val
                recordData.selectType = type

                if (recordData.sureSaveState == false) {
                    recordData.sureSavePopupShow = false
                } else if (recordData.sureSaveState == true) {
                    if (recordData.createUpdateDevcieIds.length != 0) return recordData.sureSavePopupShow = true
                }
                // recordData.loadingImg = true
                recordData.Url = config.baseUrl
                recordData.floorItem.left = 0
                recordData.floorItem.top = 0
                recordData.floorItem.lastTop = 0
                recordData.floorItem.lastLeft = 0
                recordData.floorItem.width = 1094
                recordData.floorItem.height = 670
                recordData.inheritShow = false
                recordData.sureSaveState = true
                recordData.floorsList.forEach((item) => {
                    if (item.id == val) {
                        recordData.floorId = item.id
                        recordData.floorData = item
                        recordData.floorsName = item.name
        ...
        const clrarSlect = (type) => {
            if (type == 1) {
                ...
                if(recordData.sureSavePopupShow==true){
        ...
        const change = (loopId, controllerId) => {
            if (recordData.collapseCState == true) {
                ...
                deviceListRequest(request).then((res) => {
                    ...
                    recordData.deviceList = res.data.items
        ...
        const getDeviceList = (val) => {
            let areaName = ""
            if (val.level == "area") {
                recordData.areaName = val.label
            } else if (val.level == "build") {
                recordData.buildName = val.label
            } else if (val.level == "unit") {
                recordData.unitName = val.label
            } else if (val.level == "floor") {
            ...
            if (val.children && recordData.collapseState == false) {
                ...
                if (val.children) {
                    ...
                    recordData.loadingDevice = true
                    if (val.level == "floor") {
                        request.area = recordData.areaName
                        request.build = recordData.buildName
                        request.unit = recordData.unitName
                        ...
                        request.build = recordData.buildName
                        request.unit = recordData.unitName
                        request.floor = recordData.floorname
                        ...
                        request.unit = recordData.unitName
                        request.floor = recordData.floorname
                        request.district = recordData.districtname
                    ...
                    if (recordData.deviceList.length == 0) {
                        deviceListRequest(request).then((res) => {
                            ...
                            if (recordData.deviceList[0].district == null) {
                                recordData.floororHierarchy = true//楼层
                            } else {
                                if (recordData.deviceList[0].room == null) {
                                    recordData.districtHierarchy = true//分区
                                } else {
                                    recordData.roomHierarchy = true//房间分区
                                }
                            }
                            if (recordData.deviceList) {
                                recordData.loadingDevice = false
                            ...
                            recordData.areaName = ""
                            recordData.buildName = ""
                            recordData.unitName = ""
                            recordData.floorname = ""
                            recordData.districtname = ""
                            recordData.roomname = ""
                        });
                    } else {
                        recordData.loadingDevice = false
        ...
        const dbClickAddDevice = (e) => {
            ...
            if (recordData.switchDeviceTypeActive == 2) {
                let addDevices = []
                // 当前层级为楼层时
                if (recordData.floororHierarchy == true) {
                ...
                // 当前层级为分区时
                if (recordData.districtHierarchy == true) {
                    addDevices = recordData.deviceList.filter(item => {
                ...
                // 当前层级为房间时
                if (recordData.roomHierarchy == true) {
                    addDevices = recordData.deviceList.filter(item => {
        ...
        const offSurePopup = () => {
            recordData.sureSavePopupShow = false
            recordData.floorsSlect = ""
            recordData.floorsName = ""
            recordData.stationImgUrl = ""
            recordData.sureSaveState = false
            recordData.selectedDeviceIds = []
            recordData.createUpdateDevcieIds = []
            if (recordData.selectVal != 0 && recordData.selectType != 0) {
                selectFun(recordData.selectVal, recordData.selectType)
            }

        }
        const offSureSavePopup = () => {
            recordData.sureSavePopupShow = false
            recordData.floorsSlect = recordData.floorsName
        }
        // 保存
        const openSavePopup = () => {
            recordData.sureSavePopupShow = false
        ...
        watch(() => recordData.sureSavePopupShow, (newvalue, oldvalue) => {
            if (newvalue == false) {
                recordData.sureSavePopupShow = false
                recordData.floorsSlect = recordData.floorsName
            }
